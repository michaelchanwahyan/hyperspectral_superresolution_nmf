...
cd SNR_40dB
    ...
    cd trial0001
        ...
        load( 'LOG_MFbA_HYBRID_S_FW1_A_PG1_subprobIt_1.mat' , 'A_MF'   , 'S_MF'   ) ; D2 = mean( ( Y - A_MF  *S_MF   ).^2 , 2 ) ; PSNR_MFbA_HYBRID_S_FW1_A_PG1_subprobIt_1_vs_band = 10 * log10( D1./D2 ) ;
        fid = figure ; hold on ;
        minPSNR = 30 ;
        maxPSNR = 54 ;
        k = 0 ; handle = cell(7,1) ;
        k = k + 1 ; for j = 1 : 3 ; handle{k} = semilogy( wavelength(range{j}) , PSNR_FUMI_vs_band(range{j})                                          , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal ) ; xlim([400,2500]) ; ylim([minPSNR,maxPSNR]) ; end ;
        %k = k + 1 ; for j = 1 : 3 ; handle{k} = semilogy( wavelength(range{j}) , PSNR_MFbA_gpExlnsrch_subprobIt_1_vs_band(range{j})                   , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal ) ; xlim([400,2500]) ; ylim([minPSNR,maxPSNR]) ; end ;
        %k = k + 1 ; for j = 1 : 3 ; handle{k} = semilogy( wavelength(range{j}) , PSNR_MFbA_BB_betaMax1_subprobIt_1_vs_band(range{j})                  , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal ) ; xlim([400,2500]) ; ylim([minPSNR,maxPSNR]) ; end ;
        k = k + 1 ; for j = 1 : 3 ; handle{k} = semilogy( wavelength(range{j}) , PSNR_MFbA_FW_subprobIt_1_vs_band(range{j})                           , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal ) ; xlim([400,2500]) ; ylim([minPSNR,maxPSNR]) ; end ;
        %k = k + 1 ; for j = 1 : 3 ; handle{k} = semilogy( wavelength(range{j}) , PSNR_MFbA_proxGrad_subprobIt_1_vs_band(range{j})                     , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal ) ; xlim([400,2500]) ; ylim([minPSNR,maxPSNR]) ; end ;
        k = k + 1 ; for j = 1 : 3 ; handle{k} = semilogy( wavelength(range{j}) , PSNR_MFbA_fista_subprobIt_1_vs_band(range{j})                        , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal ) ; xlim([400,2500]) ; ylim([minPSNR,maxPSNR]) ; end ;
        k = k + 1 ; for j = 1 : 3 ; handle{k} = semilogy( wavelength(range{j}) , PSNR_MFbA_HYBRID_S_FW1_A_PG1_subprobIt_1_vs_band(range{j}) , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal ) ; xlim([400,2500]) ; ylim([minPSNR,maxPSNR]) ; end ;
        ...
        %            'PG'   , ...
        %            'FISTA', ...
        %            'HYBRID'} , 'Location' , 'northeast' , 'Orientation' , 'horizontal' ) ; set( h , 'FontSize' , 10 ) ;
        h = legend( H      , ...
                  { 'FUMI' , ...
                    'FW'   , ...
                    'FISTA', ...
                    'HYBRID'} , 'Location' , 'northeast' , 'Orientation' , 'horizontal' ) ; set( h , 'FontSize' , 10 ) ;
        set( gca , 'FontSize' , 12 ) ;
        set( fid , 'Position' , [0,0,900,200] ) ;
        grid on ;
        drawnow ;
        % ----------------------------------- %
        % plot the Multispectral bands region %
        % ----------------------------------- %
        w = wavelength(  6: 12) ; outside = plot( w , (maxPSNR+0.5)*ones(1,length(w)) , 'b-' , 'LineWidth' , 3 ) ; ylim([minPSNR,maxPSNR]) ; set(outside,'Clipping','off') ;
        w = wavelength( 13: 19) ; outside = plot( w , (maxPSNR+0.5)*ones(1,length(w)) , 'b-' , 'LineWidth' , 3 ) ; ylim([minPSNR,maxPSNR]) ; set(outside,'Clipping','off') ;
        w = wavelength( 23: 32) ; outside = plot( w , (maxPSNR+0.5)*ones(1,length(w)) , 'b-' , 'LineWidth' , 3 ) ; ylim([minPSNR,maxPSNR]) ; set(outside,'Clipping','off') ;
        w = wavelength( 40: 54) ; outside = plot( w , (maxPSNR+0.5)*ones(1,length(w)) , 'b-' , 'LineWidth' , 3 ) ; ylim([minPSNR,maxPSNR]) ; set(outside,'Clipping','off') ;
        w = wavelength(114:134) ; outside = plot( w , (maxPSNR+0.5)*ones(1,length(w)) , 'b-' , 'LineWidth' , 3 ) ; ylim([minPSNR,maxPSNR]) ; set(outside,'Clipping','off') ;
        w = wavelength(150:176) ; outside = plot( w , (maxPSNR+0.5)*ones(1,length(w)) , 'b-' , 'LineWidth' , 3 ) ; ylim([minPSNR,maxPSNR]) ; set(outside,'Clipping','off') ;
