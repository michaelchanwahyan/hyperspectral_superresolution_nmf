function [ HSI_FUMI , A , S , IT_used , TIME_FUMI , returnInfo ] = HIS_FUMI_inex( HS , MS , FUMI_opt )
returnInfo.status = 'null' ;

addpath('~/Documents/Michael/HYPERSPEC/FUMI');

HS_bandNum  = FUMI_opt.HS_bandNum ;
HS_imgSizeM = FUMI_opt.HS_imgSizeM ;
HS_imgSizeN = FUMI_opt.HS_imgSizeN ;
HS_pixelNum = HS_imgSizeM * HS_imgSizeN ;
MS_bandNum  = FUMI_opt.MS_bandNum ;
MS_imgSizeM = FUMI_opt.MS_imgSizeM ;
MS_imgSizeN = FUMI_opt.MS_imgSizeN ;
MS_pixelNum = MS_imgSizeM * MS_imgSizeN ;
dsRatio     = FUMI_opt.dsRatio ;
F           = FUMI_opt.F ;
sig         = FUMI_opt.G_sigma ;
kerSiz      = FUMI_opt.G_kerSiz ;
modelOrder  = FUMI_opt.modelOrder ;
convthresh  = FUMI_opt.convthresh ;
maxIteraNum = FUMI_opt.maxIteraNum;
...
if( ~isfield( FUMI_opt , 'A_init' ) )
    error( 'ERROR : A_init is not provided !!!' ) ;
else
    A_init = FUMI_opt.A_init ;
end
para_opt_in.E_ini=A_init;
para_opt_in.It_ADMM_ABUN = FUMI_opt.It_ADMM_ABUN ;
para_opt_in.It_ADMM_EEA  = FUMI_opt.It_ADMM_EEA ;
%para_opt_in.Th_ADMM_ABUN = FUMI_opt.Th_ADMM_ABUN ;
%para_opt_in.Th_ADMM_EEA  = FUMI_opt.Th_ADMM_EEA ;

[Out,para_opt] = JointFusionUnmix(Y_H,Y_M,ChInv,CmInv,psfY,psfZ,sizeIM,para_opt_in) ;
A                 = Out.E_hyper ;
S                 = Out.A_multi ;
HSI_FUMI          = reshape( S' * A' , MS_imgSizeM , MS_imgSizeN , HS_bandNum ) ;
IT_used           = Out.IT_used ;
TIME_FUMI         = Out.TIME_FUMI ;
end
...
for t=1:N_it
    ...
    %[A,i]       = EEA(A,SG,S,Y_H,Y_M,F,LAMB_H,LAMB_M,Th_ADMM_EEA,It_ADMM_EEA); % HS+MS
    [A,i]       = EEA(A,SG,S,Y_H,Y_M,F,LAMB_H,LAMB_M); % HS+MS
    %% objective function to minimize
    cost_BCD(t) = 0.5 * norm( sqrt(LAMB_H) * (Y_H-A*SG) , 'fro' )^2 + 0.5 * norm( sqrt(LAMB_M) * (Y_M-FA*S) , 'fro' )^2 ;
...
%function [A,i] = EEA(A,SG,S,Y_H,Y_M,F,LAMB_H,LAMB_M,Th_ADMM_EEA,It_ADMM_EEA)
function [A,i] = EEA(A,SG,S,Y_H,Y_M,F,LAMB_H,LAMB_M)
%% Initialization
mu           = size(Y_H,2);
