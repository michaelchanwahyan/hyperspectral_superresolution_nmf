function [ HSI_MF , A , S , IT_used_MF , TIME_MFbA , returnInfo ] = HIS_MFbA_BBGP( HS , MS , MFbA_opt )
...
for k = 1 : maxIteraNum
    ...
    for sstep = 1 : SStep
        ...
        ADELTSG       = A * ( DELTS * G )                                   ;
        FADELTS       = FA * DELTS                                          ;
        tau           = ( - (AScurrGminY_H(:)'*ADELTSG(:))                ...
                          - (FAScurrminY_M(:)'*FADELTS(:)) )              ...
                        / ( ADELTSG(:)'*ADELTSG(:) + FADELTS(:)'*FADELTS(:) ) ;
        tau           = min( max( tau , 0 ) , 1 )                           ;
        S_curr        = S_curr + tau * DELTS                                ;
        gamm          = ADELTSG(:)'*ADELTSG(:) + FADELTS(:)'*FADELTS(:)     ;
        if( gamm > 0 ) % take beta_max as 1 , not to use 1/LS
            alphaS = min( max( DELTS(:)'*DELTS(:) / gamm , 0 ) , 1 )        ;
        else
            alphaS = 1                                                      ;
        end
        SG = S_curr * G ;
    ...
    for astep = 1 : AStep
        ...
        FDELTAS       = ( F * DELTA ) * S                                    ;
        DELTASG       = DELTA * SG                                           ;
        tau           = ( - (AcurrSGminY_H(:)'*DELTASG(:))                 ...
                          - (FAcurrSminY_M(:)'*FDELTAS(:)) )               ...
                        / ( DELTASG(:)'*DELTASG(:) + FDELTAS(:)'*FDELTAS(:) ) ;
        tau           = min( max( tau , 0 ) , 1 )                            ;
        A_curr        = A_curr + tau * DELTA                                 ;
        gamm          = DELTASG(:)'*DELTASG(:) + FDELTAS(:)'*FDELTAS(:)      ;
        if( gamm > 0 ) % take beta_max as 1 , not to use 1/LA
            alphaA = min( max( DELTA(:)'*DELTA(:) / gamm , 0 ) , 1 )         ;
        else
            alphaA = 1                                                       ;
        end
        FA = F * A_curr ;
