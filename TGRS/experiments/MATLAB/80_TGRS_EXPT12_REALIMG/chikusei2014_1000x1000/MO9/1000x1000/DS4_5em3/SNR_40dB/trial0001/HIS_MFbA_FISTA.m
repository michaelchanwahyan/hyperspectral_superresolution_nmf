...
for k = 1 : maxIteraNum
    ...
    for sstep = 1 : SStep
        ...
        if sstep>1 && SStep>20
            if( mod( sstep , 10 ) == 0 )
                hs_resi = Y_H - A*S_curr*G ;
                ms_resi = Y_M - FA*S_curr  ;
                obj_now = 0.5 * ( hs_resi(:)'*hs_resi(:) + ...
                                  ms_resi(:)'*ms_resi(:) ) ;
                hs_resi = Y_H - A*S_prev*G ;
                ms_resi = Y_M - FA*S_prev  ;
                obj_past= 0.5 * ( hs_resi(:)'*hs_resi(:) + ...
                                  ms_resi(:)'*ms_resi(:) ) ;
                obj_ch   = abs( obj_past - obj_now ) / obj_now ;
                fprintf( 'It: %d  s-it: %d subprob. obj. chng. = %f\n' , k , sstep , obj_ch ) ;
                if( obj_ch < 1e-2 )
                    break ;
                end
            end
    ...
    for astep = 1 : AStep
        ...
        if astep>1 && AStep>20
            if( mod( astep , 10 ) == 0 )
                hs_resi = Y_H - A_curr*SG ;
                ms_resi = Y_M - F*A_curr*S ;
                obj_now = 0.5 * ( hs_resi(:)'*hs_resi(:) + ...
                                  ms_resi(:)'*ms_resi(:) ) ;
                hs_resi = Y_H - A_prev*SG ;
                ms_resi = Y_M - F*A_prev*S ;
                obj_past= 0.5 * ( hs_resi(:)'*hs_resi(:) + ...
                                  ms_resi(:)'*ms_resi(:) ) ;
                obj_ch   = abs( obj_past - obj_now ) / obj_now ;
                fprintf( 'It: %d  a-it: %d subprob. obj. chng. = %f\n' , k , astep , obj_ch ) ;
                if( obj_ch < 1e-2 )
                    break ;
                end
            end
    ...
    if( SStep > 10 || AStep > 10 )
        fprintf( 'big-objective chng: %f\n' , obj_chng ) ;
    end
