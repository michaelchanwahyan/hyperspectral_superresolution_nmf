...
cd SNR_40dB
    cd trial0001
        ...
        hold on ;
        k = 0 ; handle = cell(7,1) ;
        k = k + 1 ; handle{k} = plot( time_FUMI            , 10*log10( obj_FUMI            ) , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; handle{k} = plot( time_FUMI(1:intvl_FUMI:end)          , 10*log10( obj_FUMI(1:intvl_FUMI:end)          ) , [mkstyArr{k},colorArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; handle{k} = plot( time_FUMI(1)             , 10*log10( obj_FUMI(1)            ) , [lnstyArr{k},mkstyArr{k},colorArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; yticks([0,10,20,30,40]) ; yticklabels({'10^0','10^1','10^2','10^3','10^4'}) ;
        %k = k + 1 ; handle{k} = plot( time_ExFISTA         , 10*log10( obj_ExFISTA         ) , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; handle{k} = plot( time_ExFISTA(1:intvl_ExBCD:end)      , 10*log10( obj_ExFISTA(1:intvl_ExBCD:end)      ) , [mkstyArr{k},colorArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; handle{k} = plot( time_ExFISTA(1)          , 10*log10( obj_ExFISTA(1)         ) , [lnstyArr{k},mkstyArr{k},colorArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; yticks([0,10,20,30,40]) ; yticklabels({'10^0','10^1','10^2','10^3','10^4'}) ;
        k = k + 1 ; handle{k} = plot( time_HAGO_FISTA      , 10*log10( obj_HAGO_FISTA      ) , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; handle{k} = plot( time_HAGO_FISTA(1:intvl_MF:end)      , 10*log10( obj_HAGO_FISTA(1:intvl_MF:end)      ) , [mkstyArr{k},colorArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; handle{k} = plot( time_HAGO_FISTA(1)       , 10*log10( obj_HAGO_FISTA(1)      ) , [lnstyArr{k},mkstyArr{k},colorArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; yticks([0,10,20,30,40]) ; yticklabels({'10^0','10^1','10^2','10^3','10^4'}) ;
        k = k + 1 ; handle{k} = plot( time_HAGO_SFW_AFISTA , 10*log10( obj_HAGO_SFW_AFISTA ) , [colorArr{k},lnstyArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; handle{k} = plot( time_HAGO_SFW_AFISTA(1:intvl_MF:end) , 10*log10( obj_HAGO_SFW_AFISTA(1:intvl_MF:end) ) , [mkstyArr{k},colorArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; handle{k} = plot( time_HAGO_SFW_AFISTA (1) , 10*log10( obj_HAGO_SFW_AFISTA(1) ) , [lnstyArr{k},mkstyArr{k},colorArr{k}] , 'LineWidth' , lineWidthVal , 'MarkerSize' , mkSize ) ; yticks([0,10,20,30,40]) ; yticklabels({'10^0','10^1','10^2','10^3','10^4'}) ;
        ...
        h = legend( H                            , ...
                  { 'FUMI (Exact AO by ADMM)' , ... 'Exact AO by FPG'            , ...
                    'HAGO: FPG A, FPG S'      , ...
                    'HAGO: FPG A, FW S'       } , 'Location' , 'best' ) ;
        set( h   , 'FontSize' , 18 ) ;
        set( gca , 'FontSize' , 18 ) ;
        set( fid , 'Position' , [0,0,800,400] ) ;
        xlim( [ 0 , 500 ] ) ;
        grid on ;
        hold off ;
        drawnow ;
