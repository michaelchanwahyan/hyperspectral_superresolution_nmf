function catArray = FUNC_IdentifyEndmem_UsingHungarian( A_RECOV , ENDMEM )

COST = zeros( size( A_RECOV , 2 ) , size( ENDMEM , 2 ) ) ;
for i = 1 : size( A_RECOV , 2 )
    for j = 1 : size( ENDMEM , 2 )
        COST( i , j ) = acos( norm(A_RECOV(:,i)'*ENDMEM(:,j),2) / ( norm(A_RECOV(:,i),2) * norm(ENDMEM(:,j),2) ) );
    end ; clear j
end ; clear i
[ sizeM , sizeN ] = size( COST ) ;
if    ( sizeM < sizeN )
    COST = [ COST ; repmat( max(COST(:)) , [ sizeN-sizeM , sizeN ] ) ] ;
elseif( sizeM > sizeN )
    COST = [ COST , repmat( max(COST(:)) , [ sizeM , sizeM-sizeN ] ) ] ;
end

[ catArray , ~ ] = munkres( COST ) ;
