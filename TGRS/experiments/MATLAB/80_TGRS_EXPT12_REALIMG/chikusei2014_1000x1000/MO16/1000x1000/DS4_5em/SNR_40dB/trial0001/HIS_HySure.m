function [ Zimhat , E , X , i , TIMEUSED , returnInfo ] = HIS_HySure( Yhim, Ymim, opt ) ;%downsamp_factor, R, B, p, basis_type, lambda_phi, lambda_m )
downsamp_factor = opt.dsRatio     ;
R               = opt.F           ;
B               = opt.B           ;
p               = opt.modelOrder  ;
basis_type      = opt.subspDetect ;
lambda_phi      = opt.lamb_phi    ;
lambda_m        = opt.lamb_m      ;
iters           = opt.iters       ;
...
fprintf( 'HySure: Fourier Transform of B...\n' ) ;
B = double(B);
FB = fft2(B);
FBC = conj(FB);
...
% t1 = tic;
returnInfo_time_it = zeros(iters,1) ;
returnInfo_obj_it  = zeros(iters,1) ;
returnInfo.status  = 'null' ;
tic ;
for i=1:iters
    fprintf( 'HySure It: %d / %d\n' , i , iters ) ;
    %   min   ||XB - V1 - A1||_F^2  +
    %    X    ||X  - V2 - A2||_F^2
    %         ||XDH - V3 - A3||_F^2 +
    %         ||XDV - V4 - A4||_F^2
...
%     fprintf('iter = %d out of %d\n', i, iters);
    ...
    returnInfo.time_it(i) = toc ;
end
TIMEUSED = max( returnInfo.time_it ) ;
% t2 = toc(t1);
