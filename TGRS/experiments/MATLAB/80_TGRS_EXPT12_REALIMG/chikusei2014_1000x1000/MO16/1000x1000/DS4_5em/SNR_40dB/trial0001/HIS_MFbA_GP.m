function [ HSI_MF , A , S , IT_used_MF , TIME_MFbA , returnInfo ] = HIS_MFbA_GP( HS , MS , MFbA_opt )
...
for k = 1 : maxIteraNum
    ...
    for astep = 1 : AStep
        ...
        tau            = ( - (AcurrSGminY_H(:)'*ZAminAcurr_SG(:))  ...
                           - (FAcurrSminY_M(:)'*F_ZAminAcurr_S(:)))...
                         / ( ZAminAcurr_SG(:)'*ZAminAcurr_SG(:)    ...
                           + F_ZAminAcurr_S(:)'*F_ZAminAcurr_S(:)) ;
        tau            = min( max( tau , 0 ) , 1 )             ;
