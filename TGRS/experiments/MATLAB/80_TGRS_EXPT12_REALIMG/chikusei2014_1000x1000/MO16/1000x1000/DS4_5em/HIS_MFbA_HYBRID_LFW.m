function [ HSI_MF , A , S , IT_used_MF , TIME_MFbA , returnInfo ] = HIS_MFbA_HYBRID_LFW( HS , MS , MFbA_opt )
...
for k = 1 : maxIteraNum
    ...
    for sstep = 1 : SStep
        ...
        ScurrminZS    = S_curr - ZS                              ;
        tau           = max( min( ( GRADS(:)'*ScurrminZS(:) ) / ...
                                 (( ScurrminZS(:)'*ScurrminZS(:) )*LS) , 1 ) , 0 ) ;
        S_curr        = S_curr - tau * ScurrminZS                ;
        SG            = S_curr * G                               ;
    end
    % end case 'FRANKWOLFE'
    end
