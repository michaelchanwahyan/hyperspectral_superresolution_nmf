function [ HSI_MF , A , S , IT_used_MF , TIME_MFbA , returnInfo ] = HIS_MFnAS_BB_betaMax1( HS , MS , MFbA_opt )
...
for k = 1 : maxIteraNum
    ...
    for sstep = 1 : SStep
        ...
        GRADS         = ( A' * AScurrGminY_H ) * G'                       ...
                        + FA' * FAScurrminY_M                               ;
        DELTS         = max(S_curr - alphaS*GRADS,0) - S_curr               ;
        ADELTSG       = A * ( DELTS * G )                                   ;
        FADELTS       = FA * DELTS                                          ;
    ...
    for astep = 1 : AStep
        ...
        GRADA         = AcurrSGminY_H * SG'                                ...
                        + F' * ( FAcurrSminY_M * S' )                        ;
        DELTA         = max( A_curr - alphaA*GRADA , 0 ) - A_curr            ;
        FDELTAS       = ( F * DELTA ) * S                                    ;
        DELTASG       = DELTA * SG                                           ;
